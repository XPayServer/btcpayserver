...
namespace BTCPayServer.Services.Altcoins.Ethereum.UI
{
    ...
    public class EthereumLikeStoreController : Controller
    {
        ...
        public IActionResult GetStoreEthereumLikePaymentMethod(string cryptoCode)
        {
            ...
            return View(new EditEthereumPaymentMethodViewModel()
            {
                ...
                KeyPath = string.IsNullOrEmpty( matchedPaymentMethod?.KeyPath)? network.GetDefaultKeyPath():  matchedPaymentMethod?.KeyPath
            });
        }
            ...
            EditEthereumPaymentMethodViewModel viewModel)
        {
            ...
            {
                if (
                    wallet?.GetAccount(viewModel.AddressCheck, 100) is null &&
                    !(publicWallet?.GetAddresses(100).Contains(viewModel.AddressCheck, StringComparer.InvariantCultureIgnoreCase) is true))
    ...
    public class EditEthereumPaymentMethodViewModel
    {
        ...
        [Display(Name =
            "Check wallet by providing an address it can generate within 100 indexes")]
